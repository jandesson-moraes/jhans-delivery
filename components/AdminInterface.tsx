                        ...
                        <div className="space-y-6">
                            <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-6 rounded-2xl border border-slate-700 shadow-xl relative overflow-hidden">
                                ...
                                <div className="grid grid-cols-2 gap-4 border-t border-slate-700 pt-4">
                                    <div><p className="text-xs text-slate-500 font-bold uppercase">Entregas ({currentData.deliveriesCount})</p><p className="text-lg font-bold text-white">{formatCurrency(currentData.deliveriesValue)}</p></div>
                                    <div><p className="text-xs text-slate-500 font-bold uppercase">Vales / Adiantamentos</p><p className="text-lg font-bold text-red-400">- {formatCurrency(currentData.valesValue)}</p></div>
                                </div>
                            </div>
                                        ...
                                        {currentData.orders.map(order => (
                                            <div key={order.id} className="bg-slate-900 border border-slate-800 p-4 rounded-xl shadow-sm">
                                                <div className="flex justify-between items-start">
                                                    ...
                                                    <div className="text-right whitespace-nowrap"><span className="text-emerald-500 font-bold text-sm block">{driver.paymentModel === 'fixed_per_delivery' ? formatCurrency(driver.paymentRate || 0) : driver.paymentModel === 'percentage' ? formatCurrency(order.value * ((driver.paymentRate || 0)/100)) : '-'}</span></div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            {driverSettlements.length === 0 ? <div className="text-center py-10 text-slate-500"><History size={48} className="mx-auto mb-2 opacity-20"/><p>Nenhum fechamento de caixa registrado.</p></div> : (
                                driverSettlements.map(settlement => (
                                    <div key={settlement.id} className="bg-slate-950 border border-slate-800 p-4 rounded-xl flex flex-col gap-2">
                                        <div className="flex justify-between items-center border-b border-slate-800 pb-2"><span className="text-slate-400 text-xs font-bold uppercase">{formatDate({seconds: new Date(settlement.endAt).getTime()/1000})}</span><span className="text-emerald-400 font-black text-lg">{formatCurrency(settlement.finalAmount)}</span></div>
                                        <div className="grid grid-cols-3 gap-2 text-xs text-slate-400">
                                            <div><span className="block font-bold text-white">{settlement.deliveriesCount}</span> Entregas</div>
                                            <div><span className="block font-bold text-emerald-500">{formatCurrency(settlement.deliveriesTotal)}</span> Bruto</div>
                                            <div><span className="block font-bold text-red-400">- {formatCurrency(settlement.valesTotal)}</span> Vales</div>
                                        </div>
                                    </div>
                                ))
                            )}
...
function FleetSidebar({ drivers, orders, settlements, vales, onClose, onEditDriver, onAddDriver, onSettle }: { drivers: Driver[], orders: Order[], settlements: Settlement[], vales: Vale[], onClose: () => void, onEditDriver: (d: Driver) => void, onAddDriver: () => void, onSettle: (id: string, data: any) => void }) {
    const [selectedDriverId, setSelectedDriverId] = useState<string | null>(null);
    const [showFinancialModal, setShowFinancialModal] = useState<Driver | null>(null);

    const calculateStats = (driver: Driver) => {
        const completedOrders = orders.filter((o: Order) => o.driverId === driver.id && o.status === 'completed');
        const todayOrders = completedOrders.filter((o: Order) => isToday(o.completedAt));
        let todayValue = 0, totalValue = 0;
        const rate = driver.paymentRate || 0;
        if (driver.paymentModel === 'percentage') { 
            todayValue = todayOrders.reduce((acc: number, o: Order) => acc + (o.value * (rate / 100)), 0); 
            totalValue = completedOrders.reduce((acc: number, o: Order) => acc + (o.value * (rate / 100)), 0); 
        } else if (driver.paymentModel === 'fixed_per_delivery') { 
    ...
    return (
        <>
            <div className="absolute top-4 right-4 bottom-4 w-80 bg-slate-900/95 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-700 z-[1000] flex flex-col overflow-hidden animate-in slide-in-from-right">
                <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900"><h3 className="font-black text-white text-lg flex items-center gap-2"><Bike size={20} className="text-amber-500"/> Frota ({drivers.filter((d: Driver) => d.status !== 'offline').length}/{drivers.length})</h3><div className="flex gap-2"><button onClick={onAddDriver} className="p-1.5 text-emerald-400 hover:text-white bg-emerald-900/30 hover:bg-emerald-600 rounded-lg transition-colors" title="Novo Motoboy"><PlusCircle size={18}/></button><button onClick={onClose} className="p-1.5 text-slate-400 hover:text-white bg-slate-800 rounded-lg"><X size={18}/></button></div></div>
                            ...
                            <div key={driver.id} className={`rounded-xl border transition-all relative ${isSelected ? 'bg-slate-800 border-amber-500/50' : 'bg-slate-950 border-slate-800 hover:border-slate-600'}`}>
                                ...
                                <div onClick={() => setSelectedDriverId(isSelected ? null : driver.id)} className="p-3 flex items-center gap-3 cursor-pointer group">
                                    <div className="relative">
                                        <img src={driver.avatar} className={`w-10 h-10 rounded-full border-2 object-cover ${isOnline ? (isDelivering ? 'border-amber-500' : 'border-emerald-500') : 'border-slate-600 grayscale'}`} alt={driver.name}/>
                                        <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-slate-900 flex items-center justify-center text-[8px] text-white font-bold ${isOnline ? (isDelivering ? 'bg-amber-500' : 'bg-emerald-500') : 'bg-slate-600'}`}>{isDelivering ? <Clock size={8}/> : <CheckCircle2 size={8}/>}</div>
                                    </div>
                                    <div className="flex-1 min-w-0 pr-6">
                                        <p className="font-bold text-sm text-white truncate">{driver.name}</p>
                                        <div className="flex items-center justify-between"><p className="text-[10px] text-slate-400 flex items-center gap-1">{driver.vehicle === 'Carro' ? <Car size={10}/> : <Bike size={10}/>} {driver.vehicle}{isOnline && <span className="text-emerald-500 flex items-center gap-0.5 ml-1">• GPS <Signal size={8}/></span>}</p></div>
                                    </div>
                                </div>
                                ...
                                {isSelected && (
                                    <div className="px-3 pb-3 pt-0 animate-in slide-in-from-top-2 border-t border-slate-700/50 mt-2">
                                        <div className="bg-slate-900 rounded-lg p-2 grid grid-cols-2 gap-2 border border-slate-700/50 mt-2">
                                            <div className="bg-slate-950 p-2 rounded border border-slate-800"><p className="text-[9px] text-slate-500 uppercase font-bold mb-0.5">Hoje</p><p className="text-xs font-bold text-white">{stats.todayCount} entregas</p><p className="text-xs font-black text-emerald-400">{formatCurrency(stats.todayValue)}</p></div>
                                            <div className="bg-slate-950 p-2 rounded border border-slate-800"><p className="text-[9px] text-slate-500 uppercase font-bold mb-0.5">Geral</p><p className="text-xs font-bold text-slate-300">{stats.totalCount} total</p><p className="text-xs font-bold text-slate-400">{formatCurrency(stats.totalValue)}</p></div>
                                        </div>
                                        <button onClick={() => setShowFinancialModal(driver)} className="w-full mt-3 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold py-2 rounded-lg transition-colors flex items-center justify-center gap-2 border border-slate-700 shadow-sm"><Wallet size={14}/> Financeiro & Histórico</button>
                                        <div className="flex justify-between items-center mt-3 text-[10px] text-slate-500 px-1 border-t border-slate-800/50 pt-2"><span className="flex items-center gap-1"><Battery size={10} className={driver.battery < 20 ? 'text-red-500' : 'text-slate-400'}/> {driver.battery}% Bateria</span><span>Atualizado: {formatTime(driver.lastUpdate)}</span></div>
                                    </div>
                                )}
                            </div>
                        );
...
function ManualOrderView({ products, clients, onCreateOrder, onClose, appConfig }: { products: Product[], clients: Client[], onCreateOrder: (data: any) => void, onClose: () => void, appConfig: AppConfig }) {
    ...
    const categories = useMemo(() => {
        ...
        return ['Todos', ...cats.sort((a, b) => {
             // Type safety: a and b are strings
            ...
            <div className="bg-slate-950 w-full h-full md:max-w-[95vw] md:rounded-3xl border-none md:border border-slate-800 shadow-2xl flex flex-col md:flex-row overflow-hidden relative">
                ...
                <div className={`flex-col bg-slate-900/30 border-r border-slate-800 min-w-0 min-h-0 w-full md:w-[72%] ${mobileTab === 'cart' ? 'hidden md:flex' : 'flex'}`}>
                    <div className="p-4 md:p-6 border-b border-slate-800 flex flex-col gap-4 bg-slate-950 md:bg-transparent shrink-0">
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-1 custom-scrollbar hide-scrollbar">{categories.map(cat => (<button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap transition-all border ${selectedCategory === cat ? 'bg-amber-600 border-amber-500 text-white' : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-white hover:bg-slate-800'}`}>{cat}</button>))}</div>
                    </div>
