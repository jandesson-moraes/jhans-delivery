...
import { NewOrderModal, ReceiptModal, GenericConfirmModal, EditOrderModal, DispatchSuccessModal, ProductionSuccessModal, GiveawayResponseModal } from './Modals';
    ...
    return (
        <div className="fixed inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-0 md:p-4 animate-in fade-in">
            <div className="bg-slate-900 w-full max-w-6xl h-full md:h-[90vh] rounded-none md:rounded-3xl border border-slate-800 flex flex-col md:flex-row overflow-hidden shadow-2xl relative">
                ...
                {/* --- LEFT COLUMN (CARDÁPIO) --- */}
                <div className={`w-full md:flex-1 flex-col border-r border-slate-800 shrink-0 ${mobileTab === 'products' ? 'flex h-full' : 'hidden md:flex'}`}>
                    
                    {/* Header Desktop/Mobile Unified */}
                    <div className="p-4 border-b border-slate-800 bg-slate-900 sticky top-0 z-10 flex gap-3 items-center">
                        ...
                        <div className="relative flex-1">
                            ...
                            <input className="w-full bg-slate-900 border border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-white outline-none focus:border-amber-500 transition-colors" placeholder="Buscar produto..." value={searchProduct} onChange={e => setSearchProduct(e.target.value)} />
                        </div>
                    </div>
                ...
                {/* --- RIGHT COLUMN (CLIENTE/CHECKOUT) --- */}
                <div className={`w-full md:w-96 bg-slate-950 border-l border-slate-800 flex-col md:h-full shadow-2xl shrink-0 ${mobileTab === 'checkout' ? 'flex h-full' : 'hidden md:flex'}`}>
                    
                    {/* Header Mobile Only for back context, or desktop title */}
                    <div className="p-4 border-b border-slate-800 bg-slate-900 flex items-center gap-3">
                        <h3 className="font-bold text-white text-lg">Dados do Pedido</h3>
                    </div>
                    ...
                    {/* Footer Actions - Increased Z-Index to fix unclickable button on mobile and desktop overlap */}
                    <div className="p-4 bg-slate-900 border-t border-slate-800 space-y-3 shrink-0 pb-28 md:pb-4 relative z-[70]">
                         ...
                         <button onClick={handleSubmit} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 text-base cursor-pointer"><CheckCircle2 size={20}/> Confirmar Pedido</button>
                    </div>
                </div>

                {/* --- MOBILE BOTTOM TABS (NEW) --- */}
                <div className="md:hidden absolute bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 grid grid-cols-2 z-[60] h-16 pb-safe shadow-[0_-5px_20px_rgba(0,0,0,0.5)]">
                    <button 
                        onClick={() => setMobileTab('products')}
                        className={`flex flex-col items-center justify-center gap-1 transition-colors ${mobileTab === 'products' ? 'text-amber-500 bg-slate-800/50' : 'text-slate-500'}`}
                    >
                        <Utensils size={20}/>
                    ...
                    <button 
                        ...
                        className={`flex flex-col items-center justify-center gap-1 transition-colors ${mobileTab === 'checkout' ? 'text-emerald-500 bg-slate-800/50' : 'text-slate-500'}`}
                    >
                        <div className="relative">
                            <User size={20}/>
                            {totalItems > 0 && <span className="absolute -top-2 -right-3 bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{totalItems}</span>}
                        </div>
                        <span className="text-[10px] font-bold uppercase">Dados & Finalizar</span>
                    </button>
                </div>
...
export default function AdminInterface(props: AdminProps) {
    ...
    // Estado para notificação de sorteio e modal de resposta
    // REMOVIDO: const [giveawayNotification, setGiveawayNotification] = useState<GiveawayEntry | null>(null);
    const [giveawayToRespond, setGiveawayToRespond] = useState<GiveawayEntry | null>(null);
    const prevGiveawayCount = useRef(props.giveawayEntries.length);
    ...
    useEffect(() => {
        ...
        audioRef.current = new Audio(GIVEAWAY_SOUND);
        
        // Inicializa o ref com o valor atual para não notificar na carga inicial
        prevGiveawayCount.current = props.giveawayEntries.length;
    }, [props.isMobile]);

    // Monitora novas inscrições no sorteio
    useEffect(() => {
        if (props.giveawayEntries.length > prevGiveawayCount.current) {
            // Nova entrada detectada - como ordenamos desc no app.tsx, o novo é o indice 0
            const newEntry = props.giveawayEntries[0];
            ...
            // ABRIR MODAL IMEDIATAMENTE (em vez de toast)
            setGiveawayToRespond(newEntry);
        }
        prevGiveawayCount.current = props.giveawayEntries.length;
    }, [props.giveawayEntries]);
