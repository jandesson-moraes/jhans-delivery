...
import { LayoutDashboard, Users, ShoppingBag, Utensils, Bike, Map, Settings, LogOut, FileText, BarChart3, ChevronRight, Menu as MenuIcon, X, CalendarCheck, ClipboardList, ChefHat, Bell, Gift, PlusCircle, Search, Trash2, Minus, Plus, Save, CheckCircle2, CreditCard, Banknote, MapPin, DollarSign, ClipboardPaste, Store, Navigation, Battery, MessageCircle } from 'lucide-react';
...
const createDriverIcon = (avatarUrl: string, status: string) => {
    return L.divIcon({
        className: 'custom-driver-icon',
        html: `<div class="w-10 h-10 rounded-full border-2 ${status === 'available' ? 'border-emerald-500' : 'border-amber-500'} overflow-hidden shadow-lg bg-slate-900 relative">
                <img src="${avatarUrl}" class="w-full h-full object-cover" />
                <div class="absolute bottom-0 right-0 w-3 h-3 rounded-full ${status === 'available' ? 'bg-emerald-500' : 'bg-amber-500'} border border-white"></div>
               </div>`,
        iconSize: [40, 40],
        iconAnchor: [20, 20],
        popupAnchor: [0, -20]
                    ...
                    {view === 'map' && (
                        <div className="relative w-full h-full">
                            ...
                            >
                                ...
                                {onlineDrivers.map(d => (
                                    <Marker key={d.id} position={[d.lat || 0, d.lng || 0]} icon={createDriverIcon(d.avatar, d.status)}>
                                        <Popup>
                            ...
                            <div className="absolute top-4 right-4 z-[400] w-64 bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-2xl shadow-2xl overflow-hidden max-h-[60vh] flex flex-col hidden md:flex">
                                ...
                                <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
                                    {onlineDrivers.length === 0 ? <div className="text-center py-6 text-slate-500 text-xs">Nenhum motoboy online.</div> : onlineDrivers.map(d => (
                                        <div key={d.id} className="flex items-center gap-3 p-2 rounded-xl bg-slate-950/50 border border-slate-800">
                                            <div className="relative">
                                                <img src={d.avatar} className="w-10 h-10 rounded-full object-cover border border-slate-700"/>
                                                <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-slate-900 ${d.status==='available' ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
                                            </div>
                                            ...
                                            <div className={`px-2 py-1 rounded text-[9px] font-bold uppercase ${d.status==='available' ? 'bg-emerald-900/20 text-emerald-400' : 'bg-amber-900/20 text-amber-400'}`}>
                                                {d.status === 'available' ? 'Livre' : 'Ocup'}
                                            </div>
                                        </div>
                                    ))}
                                </div>
