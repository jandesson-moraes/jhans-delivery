...
import { 
    ...
    ShoppingBag, Instagram, Calculator, TrendingUp, TrendingDown, Box, PieChart, LocateFixed, Shuffle, FlaskConical
} from 'lucide-react';
import { 
...
export function GiveawayManagerModal({ entries, onClose }: any) {
    ...
    return (
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in zoom-in">
             <div className="bg-slate-900 w-full max-w-md rounded-3xl border border-purple-500/50 p-8 text-center shadow-2xl relative overflow-hidden">
                 <button onClick={onClose} className="absolute top-4 right-4 text-slate-500 hover:text-white"><X size={20}/></button>
                 
                 <Trophy size={60} className={`mx-auto mb-4 text-amber-400 ${isAnimating ? 'animate-bounce' : ''}`}/>
                 <h2 className="text-2xl font-black text-white uppercase italic mb-6">Sorteio Oficial</h2>

                 {winner ? (
                     <div className={`bg-purple-900/20 border border-purple-500/50 p-6 rounded-2xl mb-6 ${isAnimating ? 'opacity-50' : 'animate-in zoom-in'}`}>
                         <p className="text-purple-300 text-xs font-bold uppercase mb-2">Vencedor(a)</p>
                         <h3 className="text-3xl font-black text-white mb-1">{winner.name}</h3>
                         <p className="text-slate-400 font-mono">{normalizePhone(winner.phone)}</p>
                     </div>
                 ) : (
                     <div className="p-8 mb-6 text-slate-500 border-2 border-dashed border-slate-700 rounded-2xl">
                         <Shuffle size={32} className="mx-auto mb-2 opacity-50"/>
                         <p>Clique para sortear entre {entries.length} participantes</p>
                     </div>
                 )}

                 <button onClick={pickWinner} disabled={isAnimating || entries.length === 0} className="w-full bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-bold py-4 rounded-xl shadow-lg uppercase tracking-wide">
