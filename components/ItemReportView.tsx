...
export function ItemReportView({ orders }: ItemReportProps) {
    ...
    return (
        <div className="flex-1 bg-slate-950 p-6 md:p-10 overflow-auto w-full h-full pb-20">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-slate-400">
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {reportData.length === 0 ? (
                                <tr><td colSpan={5} className="p-8 text-center text-slate-500">Nenhum item vendido neste período.</td></tr>
                            ) : (
                                reportData.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-800/50 transition-colors">
                                        <td className="p-4 font-mono text-xs">{row.date}</td>
                                        <td className="p-4 text-white font-bold">{row.product}</td>
                                        <td className="p-4 text-center text-emerald-400 font-bold">{row.qty}</td>
                                        <td className="p-4 text-xs">{row.payment}</td>
                                        <td className="p-4 text-xs truncate max-w-[150px]">{row.customer}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
