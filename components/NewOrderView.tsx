...
interface NewOrderViewProps {
    ...
    clients?: Client[]; // Adicionado para auto-complete
}
...
export function NewOrderView({ products, appConfig, onCreateOrder, clients = [] }: NewOrderViewProps) {
    ...
    const [deliveryFee, setDeliveryFee] = useState<string>(''); // Manual delivery fee
    ...
    useEffect(() => {
        ...
        if (cleanInput.length >= 8) { // Começa a buscar a partir de 8 dígitos
            const found = clients.find(c => normalizePhone(c.phone) === cleanInput);
            if (found) {
                // Só preenche o nome se o usuário ainda não tiver digitado um nome diferente ou se estiver vazio
    ...
    return (
        <div className="flex flex-col md:flex-row h-full bg-slate-950 overflow-hidden relative">
            ...
            {/* Adicionado padding bottom extra para mobile para evitar que a lista fique escondida atrás das abas */}
            <div className={`flex-1 flex-col min-w-0 ${mobileTab === 'menu' ? 'flex' : 'hidden md:flex'} pb-24 md:pb-0 h-full`}>
                ...
                {/* Products Grid - CATEGORIZED & COMPACT */}
