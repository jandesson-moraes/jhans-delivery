...
import { Battery, Bike, DollarSign, ShoppingBag, Clock, User, Signal, Wallet, MapPin } from 'lucide-react';
...
const DriverCard: React.FC<DriverCardProps> = React.memo(({ driver, orders, vales = [], onEdit, onCloseCycle }) => {
    // Cálculos Financeiros por Motorista
    const financial = useMemo(() => {
        const lastSettlement = driver.lastSettlementAt?.seconds || 0;
        
        // Pedidos entregues por este motorista desde o último acerto
        ...
        // Vales retirados desde o último acerto
    ...
    // Definição de Cores baseadas no Status
    const isOnline = driver.status !== 'offline';
    const isDelivering = driver.status === 'delivering';
    
    const statusColor = isDelivering ? 'text-amber-500 bg-amber-500/10 border-amber-500/20' 
                      : isOnline ? 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20'
                      : 'text-slate-500 bg-slate-800 border-slate-700';

    const statusLabel = isDelivering ? 'Entregando' 
                      : isOnline ? 'Disponível'
                      : 'Offline';
    ...
    return (
        <div 
            onClick={onEdit}
            className={`relative rounded-2xl border p-4 transition-all hover:border-slate-500 cursor-pointer group flex flex-col justify-between backdrop-blur-md ${!isOnline ? 'bg-slate-950/60 border-slate-800 opacity-60 grayscale-[0.5]' : 'bg-slate-900/80 border-slate-700/50 shadow-xl'}`}
        >
            {/* Header do Card */}
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h4 className="font-bold text-white text-base leading-tight">{driver.name}</h4>
...
export function MonitoringView({ 
    ...
    const stats = useMemo(() => {
        ...
        // Filtra motoristas online (status diferente de offline)
    ...
    // Ordenação: Online primeiro, depois por última atualização
    const sortedDrivers = useMemo(() => {
        return [...drivers].sort((a, b) => {
            const statusOrder = { delivering: 3, available: 2, offline: 1 };
    ...
    return (
        <div className="flex-1 bg-slate-950 h-full flex flex-col overflow-hidden relative">
            ...
            <div 
                ...
                style={{
                    backgroundImage: "url('https://images.unsplash.com/photo-1524661135-423995f22d0b?q=80&w=1974&auto=format&fit=crop')", // Mapa escuro/Cidade
                    backgroundSize: 'cover',
                    backgroundPosition: 'center',
                    filter: 'grayscale(100%) contrast(1.1) brightness(0.6)'
                }}
            />
            {/* Vignette para focar o conteúdo e escurecer as bordas */}
            ...
            {/* --- CONTEÚDO PRINCIPAL (z-10 para ficar acima do mapa) --- */}
